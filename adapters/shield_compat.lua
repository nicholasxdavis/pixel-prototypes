...
function Compat.rollNewShield(opts)
  opts = opts or {}
  local archetype = opts.archetype or opts.arch
  local asset, err = MTE.random("shield", {
    archetype = archetype,
    id = opts.id,
    seed = opts.seed,
  ...
  local resolved_arch = meta.archetype or archetype or "shield"
  ...
  return {
    ...
    element = meta.element or nil,
    name = title_case((resolved_arch or "shield"):gsub("_", " ")),
  }
end
